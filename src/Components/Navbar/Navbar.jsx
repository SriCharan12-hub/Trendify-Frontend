  ...
  return (
    <div>
      <header className="navbar">
       ...
       {
        (!window?.location?.pathname?.includes('/homepage') &&  !window?.location?.pathname?.includes('/userdashboard') && !window?.location?.pathname?.includes('/orderconfirmed') && !window?.location?.pathname?.includes('/checkout') && !window?.location?.pathname?.includes('/cart')) && (
          <div className="search-container">
            <svg className="search-icon" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" onClick={() => {
                const q = searchTerm.trim();
                window.dispatchEvent(new CustomEvent('site-search', { detail: { query: q } }));
              }} style={{ cursor: 'pointer' }}>
              <path d="M21 21L15 15M17 10C17 13.866 13.866 17 10 17C6.13401 17 3 13.866 3 10C3 6.13401 6.13401 3 10 3C13.866 3 17 6.13401 17 10Z" stroke="#7f8c8d" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
            <input
              type="text"

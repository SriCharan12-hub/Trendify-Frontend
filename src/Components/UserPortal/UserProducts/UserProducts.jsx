...
const UserProducts = () => {
    ...
    useEffect(() => {
        const fetchProductsAndUserContent = async () => {
            ...
            try {
                const productResponse = await axios.get(`${import.meta.env.VITE_API_URL}/products/get`,{ headers: { Authorization: `Bearer ${token}` } });
                ...
                if (token) {
                    ...
                    axios.get(`${import.meta.env.VITE_API_URL}/cart`, { headers: { Authorization: `Bearer ${token}` } }),
                        axios.get(`${import.meta.env.VITE_API_URL}/wishlist/get`, { headers: { Authorization: `Bearer ${token}` } }),
                    ]);
    ...
    // listen for site-search events from the header (in-place search)
    useEffect(() => {
        const handler = (e) => {
    ...
    useEffect(() => {
        ...
        let newFilteredProducts = [...electronicsProducts];
        ...
        setCurrentPage(1);
    }, [electronicsProducts, selectedPriceRange, sortOrder, searchQuery]);
    ...
    const handleAddToWishlist = async (product) => {
        ...
        try {
            await axios.post(`${import.meta.env.VITE_API_URL}/wishlist/add`, { productId }, {
                headers: { Authorization: `Bearer ${token}` }
            });
        } catch (err) {

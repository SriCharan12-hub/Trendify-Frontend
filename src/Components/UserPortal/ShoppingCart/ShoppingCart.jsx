...
const ShoppingCart = () => {
    ...
    const handleIncrement = async (productId, currentQuantity) => {
        ...
        try {
            await axios.put(`${import.meta.env.VITE_API_URL}/cart/item`, { productId, quantity: newQuantity }, { headers: { Authorization: `Bearer ${token}` } });
    ...
    const handleDecrement = async (productId, currentQuantity) => {
        ...
        try {
            if (newQuantity > 0) {
                ...
                await axios.delete(`${import.meta.env.VITE_API_URL}/cart/item`, { headers: { Authorization: `Bearer ${token}` }, data: { productId } });
    ...
    return (
        <div className="cart-container">
            <button className="back-btn" onClick={() => { navigate('/homepage'); window.scrollTo(0, 0); }}>Back</button>
            {/* <NavSearch />  */}
            ...
            {cartItems.length === 0 ? (
                ...
                <>
                    ...
                    <div className="summary-section">
                        ...
                        <button className="checkout-btn" onClick={() => { navigate('/checkout'); window.scrollTo(0, 0); }}>Proceed to Checkout</button>
                    </div>
                </>
            )}
        </div>
